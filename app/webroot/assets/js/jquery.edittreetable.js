...
(function($){
	$.fn.bstreetable = function(options){
		...
		var settings = {
            ...
            text:{
                NodeDeleteText:"Bạn chắc chắn xóa bản ghi đó không?"
            }
		};
		var TREENODECACHE = "treenode";
		var language ={};
		language.addchild = "Thêm một trường";
		if(options) {           
        ...
        /*render data*/
        var dom_addFirstLevel = $("<div class='tt-operation m-b-sm'></div>").append($("<button class='btn btn-primary btn-sm j-addClass'><i class='fa fa-level-down'></i>&nbsp;Thêm trường mới</button>"));
        ...
        element.delegate(".j-remove","click",function(event){
            ...
            if(parentDom.attr("data-loaded")=="true"){
                if(parentDom.parent().find(".class-level").length>0){
                    settings.customalert("Không thể xóa!");
                    return;
                }
                else{
                    isRemoveAble = true;
